    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0 space-y-4">
		  ...
		  {detail.workflow_state === "Diagnosticado — aguardando orçamento" && detail.diagnosis.pricing_responsibility === "Técnico" ? <TechnicalBudgetEditor estimatedDeadline={detail.estimated_deadline} onCompleted={() => onRefresh("Orçamento concluído e encaminhado para aprovação.")} onToast={onToast} serviceOrder={detail.name} /> : null}
          <TimelineCard events={detail.timeline} />
          </> : null}
...
function TechnicalBudgetEditor({ estimatedDeadline, onCompleted, onToast, serviceOrder }: { estimatedDeadline: string; onCompleted: () => Promise<void>; onToast: (message: string, tone?: ToastState["tone"]) => void; serviceOrder: string }) {
	...
	return <Card className="p-5">
		...
		<label className="mt-3 flex flex-col gap-1"><span className="text-xs font-bold uppercase tracking-wide text-tec-muted">Prazo estimado</span><div className="flex gap-2"><input className="tp-input" disabled={suggesting} min={new Date().toISOString().slice(0, 10)} onChange={(event) => setDeadline(event.target.value)} placeholder={suggesting ? "Calculando sugestão…" : "Defina o prazo estimado"} type="date" value={deadline} /><Button disabled={busy || suggesting || !deadline} onClick={async () => { setBusy(true); try { const response = await serviceOrders.setEstimatedDeadline(serviceOrder, deadline); setDeadline(response.estimated_deadline); onToast("Prazo estimado salvo."); } catch (caught) { onToast(caught instanceof Error ? caught.message : "Não foi possível salvar o prazo.", "error"); } finally { setBusy(false); } }}>Salvar prazo</Button></div></label>
