...
export function AdministrationSettingsScreen({ activeTechnicians, onBack, onToast }: { activeTechnicians: number; onBack: () => void; onToast: Toast }) {
  ...
  const updateOperation = (key: string, value: boolean | number | string) => setSettings((current) => current ? { ...current, operation: { ...current.operation, [key]: value } } : current);
