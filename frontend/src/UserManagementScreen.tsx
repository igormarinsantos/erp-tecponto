...
export function UserManagementScreen({ onToast }: { onToast: Toast }) {
  ...
  const resetPassword = async (item: ManagedUserAccount) => {
    setResetting(item.name);
    try {
      await userAccounts.sendPasswordReset(item.name);
      onToast(`O fluxo nativo de redefinição foi enviado para ${item.email}.`);
    } catch (error) {
      onToast(error instanceof Error ? error.message : "Não foi possível enviar a redefinição.", "error");
    } finally {
      setResetting(null);
    }
  };
