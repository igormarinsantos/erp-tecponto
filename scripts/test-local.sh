...
docker run --rm --network host \
	-e DB_PORT="$DB_PORT" -e REDIS_PORT="$REDIS_PORT" -e SITE_EXISTS="$SITE_EXISTS" \
	-v "$ROOT:/home/frappe/frappe-bench/apps/tecponto_app:ro" \
...
if [[ "$SITE_EXISTS" != "1" ]]; then
  ...
  ./env/bin/python -c "import os, frappe; os.chdir(\"/home/frappe/frappe-bench/sites\"); frappe.init(site=\"local-ci.local\", sites_path=\"/home/frappe/frappe-bench/sites\"); frappe.connect(); frappe.set_user(\"Administrator\"); from frappe.desk.page.setup_wizard.setup_wizard import setup_complete; setup_complete(\"{\\\"country\\\":\\\"Brazil\\\",\\\"company_name\\\":\\\"Tecponto\\\",\\\"company_abbr\\\":\\\"TEC\\\",\\\"currency\\\":\\\"BRL\\\",\\\"timezone\\\":\\\"America/Sao_Paulo\\\",\\\"fy_start_date\\\":\\\"2026-01-01\\\",\\\"fy_end_date\\\":\\\"2026-12-31\\\",\\\"chart_of_accounts\\\":\\\"Standard\\\"}\"); from tecponto_app.install import bootstrap_erpnext_foundation; bootstrap_erpnext_foundation(); frappe.db.commit(); frappe.destroy()"
